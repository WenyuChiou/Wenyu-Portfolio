...
function initApp() {
  ...
  // setupChatWidget(); // Disabled per user request
...
function setupTimelineNavigation() {
  ...
  if (timelineItems.length > 0) {
    const firstId = timelineItems[0].getAttribute('data-exp-id');
    showExperience(firstId);
  }

  // Close Detail Logic
  closeBtn.addEventListener('click', () => {
    detailPanel.classList.remove('visible');
    setTimeout(() => detailPanel.classList.remove('active'), 300);
  });
}
...
function setupChatWidget() {
  ...
  // Hardcoded API Key (Safe for free tier static sites)
  const API_KEY = ''; // API Key removed for security

  if (!toggleBtn || !chatWindow || !input || !messagesContainer) return;
  ...
  // Build System Context from Website Data
  function getSystemContext() {
    const data = window.contentData;
    if (!data) return "You are a helpful assistant.";

    const profile = data.personal;
    const about = data.about.paragraphs.en.join(' ');
    const skills = data.skills.categories.map(c => `${c.title.en}: ${c.items.join(', ')}`).join('. ');

    // Recent Projects (Take top 3)
    const projects = data.projects.items.slice(0, 3).map(p => `${p.name.en}: ${p.description.en}`).join('. ');

    return `You are an AI assistant for Wenyu Chiou's personal portfolio website.
    here is Wenyu's profile:
    ...
    Instructions:
    - Answer questions as if you are a helpful guide on his website.
    - Be concise, professional, and friendly.
    - If asked about something not in the profile, politely say you don't know but suggest checking the "Contact" section.
  ...
  async function sendMessage() {
    ...
    // Note: The API integration is currently commented out due to Hugging Face permission changes.
    // To enable real API: Uncomment the block below and ensure your Token has Inference permissions.

    setTimeout(() => {
      removeLoadingIndicator(loadingId);
      const responses = [
        "That's a great question! I'm currently in Demo Mode, but I can tell you Wenyu specializes in Agent-Based Modeling.",
        "Wenyu is a PhD student at Lehigh University. (Demo Response)",
        "His research focuses on flood adaptation strategies using ABM and LLMs. (Demo Response)",
        "Please check the 'Publications' section for his latest papers! (Demo Response)"
      ];
      // Simple context-aware check
      let reply = responses[Math.floor(Math.random() * responses.length)];
      if (text.toLowerCase().includes('who')) reply = "I am Wenyu's AI Assistant (Demo Version). Wenyu is a researcher at Lehigh University.";

      addMessage(reply, 'ai');
    }, 1500);

    /* 
    // REAL API INTEGRATION (Requires valid Router Permissions)
    try {
      ...
      const response = await fetch("https://router.huggingface.co/hf-inference/models/mistralai/Mistral-7B-Instruct-v0.3", {
        ...
        body: JSON.stringify({
          inputs: `<s>[INST] ${getSystemContext()}\n\nUser: ${text} [/INST]`,
          parameters: {
            max_new_tokens: 250,
            temperature: 0.7,
      ...
      addMessage("Error connecting to AI. Please try again later.", 'error');
    }
    */
  }
